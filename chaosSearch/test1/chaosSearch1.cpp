...
#define inName "../TSP/rbg443a.txt"
#define outName "out.txt" //書き出したいファイル名
#define cityNum 443
#define twoOptTimes 10000 //2optで何回最小値を出すか,最小値を出すまでループで減らない
...
int main(void)
{
	...
	beforeSum += edge[city[cityNum - 1]][city[0]];
	out << beforeSum << endl;
	out << "\n";
	...
	do
	{
		...
		do //2optができる2点をランダムに生成
		{
			...
			r2 = dist2(engine); //ランダム生成点2
								//	out << "Set point TIME:" << i + 1 << "\t" //何回ランダム生成したかを出力
								//		<< "point:"
								//		<< "\t" << r1 << "\t" << r2 << endl; //出力された点を表示
			i++;
		} while ((r1 == r2) || (r1 == 0 && r2 == cityNum - 1) || (r2 == 0 && r1 == cityNum - 1) || (abs(r1 - r2) == 1)); //同じ点でなく、始点と終点でなく、2点の差の絶対値が1より大きくなるまでランダム生成
		...
		if (r1 == cityNum - 1)
		{
			range1 = edge[r1][0] + edge[r2][r2 + 1]; //繋ぎ変える前
			range2 = edge[r1][r2] + edge[0][r2 + 1]; //繋ぎ変えた後
		}
		else if (r2 == cityNum - 1)
		{
			range1 = edge[r1][r1 + 1] + edge[r2][0]; //繋ぎ変える前
			range2 = edge[r1][r2] + edge[r1 + 1][0]; //繋ぎ変えた後
		}
		else
		{
		...
		{
			// out << "[OK]range1 = " << range1 << "\trange2 = " << range2 << endl;
			...
			sum += edge[city[cityNum - 1]][city[0]];
		...
		{
			// out << "[NG]range1 = " << range1 << "\trange2 = " << range2 << endl;
			// out << "debug:range1 <= range2" << endl;
		}
